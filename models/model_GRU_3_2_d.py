...
CHECKPOINT_PATH = 'models/log7/check_points/'
LOGDIR = 'models/log7/'
LEARNING_RATE = 0.001
BATCH_SIZE = 256
MAX_STEP = 40
...
def train(inputs, epochs, batch_size = BATCH_SIZE):
	...
	with tf.variable_scope('dense1') as scope:
		w = tf.Variable(tf.truncated_normal([4*N_HIDDEN_3,N_HIDDEN_DENSE], stddev=0.1, dtype=tf.float32), name='weights1')
		b = tf.Variable(tf.zeros([N_HIDDEN_DENSE], dtype=tf.float32), name="bias1")
	...
	with tf.variable_scope('dense2') as scope:
		w = tf.Variable(tf.truncated_normal([N_HIDDEN_DENSE,1], stddev=0.1, dtype=tf.float32), name='weights2')
		...
		while epochs>0:
			...
			if int_step % 100 == 0:
				...
				train_accuracy = sess.run(accuracy, feed_dict={in1: batch['vec1'],
					target: batch['is_duplicate'], in2: batch['vec2'], keep_prob: KEEP_PROB})#, overlap: batch['overlap']})
			...
			_,summary = sess.run([train_step,summary_op], feed_dict={in1: batch['vec1'], in2: batch['vec2'],
				target: batch['is_duplicate'], keep_prob: KEEP_PROB})#, overlap: batch['overlap']})
